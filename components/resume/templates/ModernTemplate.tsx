...
const LinkIcon     = () => <svg className="w-[9px] h-[9px] fill-none stroke-current inline-block align-middle ml-0.5 translate-y-[1px]" style={{color:'#4f46e5'}} strokeWidth="3" viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>;

const ACCENT = '#4f46e5'; // indigo-600

const SectionTitle = ({ children }: { children: React.ReactNode }) => (
  <h2
    className="text-[11px] font-bold uppercase tracking-[0.12em] mb-0.5 pb-1 leading-snug"
        ...
        if (secId === 'keyAchievements' && (sections.keyAchievements?.items?.length ?? 0) > 0) {
          return (
            <section key={secId} className="mb-2">
              ...
              <ul className="text-[10.5px] text-[#374151] leading-[1.45] list-none pl-0 space-y-1 [&_li]:relative [&_li]:pl-3.5 [&_li]:mt-0.5 [&_li::before]:content-['•'] [&_li::before]:absolute [&_li::before]:left-0 [&_li::before]:top-[0.5px] [&_li::before]:font-sans [&_li::before]:font-bold [&_li::before]:text-indigo-600">
        ...
        if (secId === 'certifications' && (((sections.achievements?.items?.length ?? 0) > 0 || (sections.certifications?.items?.length ?? 0) > 0))) {
          return (
            <section key={secId} className="mb-2">
              ...
              <ul className="text-[10.5px] text-[#374151] list-none pl-0 space-y-[2px] leading-[1.45] [&_li]:relative [&_li]:pl-3.5 [&_li]:mt-0.5 [&_li::before]:content-['•'] [&_li::before]:absolute [&_li::before]:left-0 [&_li::before]:top-[0.5px] [&_li::before]:font-sans [&_li::before]:font-bold [&_li::before]:text-indigo-600">
