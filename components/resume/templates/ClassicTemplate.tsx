...
const LinkIcon     = () => <svg className="w-[9px] h-[9px] fill-none stroke-current text-blue-600 inline-block align-middle ml-0.5 translate-y-[1px]" strokeWidth="3" viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>;

// ─── Section Title ────────────────────────────────────────────────────────────
const SectionTitle = ({ children }: { children: React.ReactNode }) => (
  <h2 className="text-[12px] font-bold uppercase tracking-wider text-gray-950 mb-0.5 border-b border-gray-950 pb-1 font-serif leading-snug">
    {children}
  </h2>
);
        ...
        if (secId === 'keyAchievements' && (sections.keyAchievements?.items?.length ?? 0) > 0) {
          return (
            <section key={secId} className="mb-2">
              ...
              <ul className="text-[10.5px] text-gray-800 leading-[1.45] list-none pl-0 space-y-1 [&_li]:relative [&_li]:pl-3.5 [&_li]:mt-0.5 [&_li::before]:content-['•'] [&_li::before]:absolute [&_li::before]:left-0 [&_li::before]:top-[0.5px] [&_li::before]:font-sans [&_li::before]:font-bold [&_li::before]:text-gray-950">
        ...
        if (secId === 'certifications' && (((sections.achievements?.items?.length ?? 0) > 0 || (sections.certifications?.items?.length ?? 0) > 0))) {
          return (
            <section key={secId} className="mb-2">
              ...
              <ul className="text-[10.5px] text-gray-800 list-none pl-0 space-y-[2px] leading-[1.4] [&_li]:relative [&_li]:pl-3.5 [&_li]:mt-0.5 [&_li::before]:content-['•'] [&_li::before]:absolute [&_li::before]:left-0 [&_li::before]:top-[0.5px] [&_li::before]:font-sans [&_li::before]:font-bold [&_li::before]:text-gray-950">
