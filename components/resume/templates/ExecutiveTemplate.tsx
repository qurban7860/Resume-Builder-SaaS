...
const sz = 'w-[10px] h-[10px] inline-block mr-[3px] flex-shrink-0 translate-y-[1.5px]';
const LocationIcon = () => <svg className={`${sz} fill-none stroke-current`} strokeWidth="2" viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>;
const PhoneIcon    = () => <svg className={`${sz} fill-none stroke-current`} strokeWidth="2" viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>;
const EmailIcon    = () => <svg className={`${sz} fill-none stroke-current`} strokeWidth="2" viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>;
...
const WebsiteIcon  = () => <svg className={`${sz} fill-none stroke-current`} strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>;
const LinkIcon     = () => <svg className="w-[8px] h-[8px] fill-none stroke-current inline-block align-middle ml-0.5 opacity-60 translate-y-[1px]" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>;

const SERIF = 'Georgia, "Times New Roman", Times, serif';
const DARK  = '#1a0a2e'; // deep plum-black for name
const MID   = '#3b0764'; // purple-mid
const MUTED = '#6b7280';

// Centered section title with decorative horizontal rules
const SectionTitle = ({ children }: { children: React.ReactNode }) => (
  <div className="flex items-center gap-2 mb-1.5">
    <div className="flex-1 h-px bg-gray-300" />
    <h2
      className="text-[10.5px] font-semibold uppercase tracking-[0.14em] whitespace-nowrap px-1"
      style={{ color: MID, fontFamily: SERIF }}
    >
      {children}
    </h2>
    <div className="flex-1 h-px bg-gray-300" />
    ...
    <div className="bg-white" style={{ fontFamily: SERIF, color: '#2d2d2d' }}>
      ...
      <header className="flex items-start justify-between mb-2 pb-2" style={{ borderBottom: '1.5px double #6b21a8' }}>
        ...
        <div className="flex-1 pr-4">
          ...
          {basics.headline && (
            <div style={{ fontSize: '11.5px', fontWeight: 600, color: MID, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
              {basics.headline}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col items-end gap-[3px]" style={{ fontSize: '10px', color: MUTED, flexShrink: 0 }}>
          {contactItems.map((item, i) =>
            item.href ? (
              <a key={i} href={item.href} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-0.5 hover:underline" style={{ color: MID }}>
                {item.icon}<span>{item.label}</span>
              </a>
            ) : (
              <span key={i} className="inline-flex items-center gap-0.5" style={{ color: MUTED }}>
                {item.icon}<span>{item.label}</span>
              </span>
            )
          )}
        ...
        if (secId === 'keyAchievements' && (sections.keyAchievements?.items?.length ?? 0) > 0) {
          return (
            <section key={secId} className="mb-2">
              ...
              <ul className="text-[10.5px] text-[#374151] leading-[1.45] list-none pl-0 space-y-1 [&_li]:relative [&_li]:pl-3.5 [&_li]:mt-0.5 [&_li::before]:content-['•'] [&_li::before]:absolute [&_li::before]:left-0 [&_li::before]:top-[0.5px] [&_li::before]:font-sans [&_li::before]:font-bold [&_li::before]:text-[#3b0764]">
        ...
        if (secId === 'certifications' && (((sections.achievements?.items?.length ?? 0) > 0 || (sections.certifications?.items?.length ?? 0) > 0))) {
          return (
            <section key={secId} className="mb-2">
              ...
              <ul className="text-[10.5px] text-[#374151] list-none pl-0 space-y-[2px] leading-[1.5] [&_li]:relative [&_li]:pl-3.5 [&_li]:mt-0.5 [&_li::before]:content-['•'] [&_li::before]:absolute [&_li::before]:left-0 [&_li::before]:top-[0.5px] [&_li::before]:font-sans [&_li::before]:font-bold [&_li::before]:text-[#3b0764]">
