...
export const ResumeEditor = () => {
  ...
  const handleAchievementChange = (index: number, field: 'title' | 'subtitle' | 'startDate' | 'endDate', value: string) => {
    if (!resume) return;
  ...
  const renderAccordionHeader = (
    ...
    return (
      <button
        ...
        className="w-full text-left flex items-center justify-between p-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-2xl transition-all duration-200 hover:bg-slate-50/60"
      >
        ...
        <div className="flex items-center gap-3">
          {badgeText && (
            <span className="text-[10px] font-bold px-2.5 py-0.5 bg-slate-100/70 border border-slate-200/50 text-slate-500 rounded-full">
              {badgeText}
            </span>
          )}
          <svg
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className={`rounded-2xl border transition-all duration-300 ${
        ...
        <div className="flex items-center justify-between pr-4">
          <div className="flex-1">
            {renderAccordionHeader(
              ...
              `${(resume.sections.relevantCoursework?.items || []).length} courses`
            )}
          </div>
          {expandedSection === 'coursework' && (
            <button
        ...
        <div className={`accordion-body ${expandedSection === 'coursework' ? 'open' : ''}`}>
          <div>
            <div className="border-t border-slate-100 p-5 bg-white rounded-b-2xl space-y-4">
            {(resume.sections.relevantCoursework?.items || []).length === 0 ? (
              <p className="text-center py-6 text-xs font-medium text-slate-400">No courses listed. Click 'Add' to record coursework.</p>
            ) : (
              <div className="space-y-4">
                {(resume.sections.relevantCoursework?.items || []).map((item: any, index: number) => (
                  ...
                  >
                    ...
                    <div>
                      <label className={premiumLabelClass}>Course Name</label>
                      <input
                        className={premiumInputClass}
                        value={item.name}
                        placeholder="e.g. Distributed Systems or Neural Networks"
      ...
      <div className={`rounded-2xl border transition-all duration-300 ${
        ...
        <div className="flex items-center justify-between pr-4">
          <div className="flex-1">
            {renderAccordionHeader(
              ...
              `${(resume.sections.achievements?.items || []).length} items`
            )}
          </div>
          {expandedSection === 'achievements' && (
            <button
        ...
        <div className={`accordion-body ${expandedSection === 'achievements' ? 'open' : ''}`}>
          <div>
            <div className="border-t border-slate-100 p-5 bg-white rounded-b-2xl space-y-4">
            {(resume.sections.achievements?.items || []).length === 0 ? (
              <p className="text-center py-6 text-xs font-medium text-slate-400">No achievements added. Click 'Add' to record honors.</p>
            ) : (
              <div className="space-y-6">
                {(resume.sections.achievements?.items || []).map((item: any, index: number) => (
                  ...
                  >
                    ...
                    <div className="grid gap-4">
                      <div>
                        <label className={premiumLabelClass}>Achievement Title</label>
                        <input
                          className={premiumInputClass}
                          value={item.title}
                          placeholder="e.g. Dean's Honor List or 1st Place Hackathon"
                          onChange={(event) => handleAchievementChange(index, 'title', event.target.value)}
                        />
                      </div>
                      <div>
                        <label className={premiumLabelClass}>Subtitle / Issuer</label>
                        <input
                          className={premiumInputClass}
                          value={item.subtitle || ''}
                          placeholder="e.g. PUCIT or Google Developer Group"
                          onChange={(event) => handleAchievementChange(index, 'subtitle', event.target.value)}
                        />
                      </div>
                      <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                          <label className={premiumLabelClass}>Start Date / Year</label>
                          <input
                            className={premiumInputClass}
                            value={item.startDate}
                        ...
                        <div>
                          <label className={premiumLabelClass}>End Date / Year (Optional)</label>
                          <input
                            className={premiumInputClass}
                            value={item.endDate || ''}
